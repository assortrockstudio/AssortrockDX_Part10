...
public:
    ...
    Ptr<CTexture>   CreateTexture(const wstring& _strKey, const vector<Ptr<CTexture>>& _vecTex);


    template<typename T>
    Ptr<T> Load(const wstring& _strKey, const wstring& _strRelativePath = L"");
